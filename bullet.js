...
function bulletMove() {
    ...
    bullets.forEach(function(bullet, i, array) {
        ...
        //remove undefined (e.g. deleted) enemy, bullet and coin elements from their array. This will make it easier to count total enemy numbers and total bullet numbers.
        enemies = enemies.filter(item => item !== undefined);
        bullets = bullets.filter(item => item !== undefined);
        coins = coins.filter(item => item !== undefined);
    });
} //end of function bulletMove()



//function for drawing bullets using colorRect(x,y,w,h,c)
